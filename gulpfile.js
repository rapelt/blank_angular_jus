...
var paths = {
  ...
  styles: [yeoman.app + '/features/**/*.scss', yeoman.app + '/features/common/font-awesome.min.css'],
...
gulp.task('e2e-phantom', ['start:server:test', 'index'], function (cb) {
  runProtractor({
...
gulp.task('e2e', [ 'start:server:test', 'eslint', 'index' ], function (cb) {
  runProtractor({
    'configFile': './E2E/conf.js',
    'autoStartStopServer': true,
    'debug': false,
...
gulp.task('e2e-QA', [ 'eslint', 'index' ], function (cb) {
  runProtractor({
    'configFile': './E2E/conf.js',
    'autoStartStopServer': true,
    'debug': false,
...
gulp.task('client:build', ['html', 'images', 'index'], function () {
...
gulp.task('build', ['clean:dist', 'styles'], function () {
  runSequence(['eslint', 'index', 'images', 'config', 'copy:extras', 'copy:fonts', 'copy:json', 'copy:fontAwesome', 'copy:swe-templates', 'copy:ng-templates', 'client:build']);
});
